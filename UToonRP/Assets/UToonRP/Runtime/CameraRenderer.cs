...
public partial class CameraRenderer
{
    ...
    private static int depthBufferId = Shader.PropertyToID("_CameraDepthBuffer");
    ...
    , ShadowSettings shadowSettings, PostFXSettings postFXSettings)
    {
        ...
        //配置后处理堆栈
        postFXStack.Setup(context, camera, postFXSettings, cullingResults, frameBufferId, depthBufferId);
    ...
    void Setup()
    {
        ...
        if (postFXStack.IsActive)
        {
            ...
            buffer.GetTemporaryRT(depthBufferId, camera.pixelWidth, camera.pixelHeight, 32, FilterMode.Point,
                RenderTextureFormat.Depth);
            //设置RT为RenderTarget
            buffer.SetRenderTarget(frameBufferId, RenderBufferLoadAction.DontCare, RenderBufferStoreAction.Store,
                depthBufferId, RenderBufferLoadAction.DontCare, RenderBufferStoreAction.Store);
            // buffer.SetRenderTarget(frameBufferId, depthBufferId);
            // buffer.SetRenderTarget(frameBufferId, RenderBufferLoadAction.DontCare, RenderBufferStoreAction.Store);
        }
